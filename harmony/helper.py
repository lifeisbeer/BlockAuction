...
def deploy_contract(w3, contract_path):
    ...
    print("Contract deployed at address: {} and block: {}."
          .format(tx_receipt.contractAddress, tx_receipt.blockNumber))
...
def validate_commitment(w3, assets, address, commitment, ciphertext, secret, type):
    #if commitment == hashlib.sha3_256(ciphertext).digest():
    if commitment == w3.solidityKeccak(['bytes'], [ciphertext]):
        try:
