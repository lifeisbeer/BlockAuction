...
while (True):
    ...
    order_hash = w3.solidityKeccak(['bytes'], [order_ciphertext])
    ...
    tx = darkPool.functions.commit_order(order_hash).buildTransaction({
        ...
        'gasPrice': 100000000000 #1000000000 # from truffle docs
    })
    # sign transaction
    ...
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash, timeout=3000)
    ...
    for order in executed:
        ...
        # validate commitment
        status, order = validate_commitment(w3, assets, sender, commitment, ciphertext, secret, False)
    ...
    fraud = darkPool.events.fraudClaimed.getLogs(fromBlock=start)
    for f in fraud:
