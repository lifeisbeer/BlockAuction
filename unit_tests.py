...
gas_verbose = True # Gas cost messages
...
if gas_verbose: print("Gas used:", tx_receipt.gasUsed)
...
# Unit Tests
# ------------------------------------------------------------------------------
''' 154 different unit tests, but actual number of tests that are going to be
    runned depends on set client number (eg 217 for 2 clients) '''
...
for i in range(2, clients+4):
    ...
    test_num += 1
    assert darkPool.functions.orders(addr).call()[0] == b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', "Test {} failed: Commitment not initialised correctly.".format(test_num)
    test_num += 1
...
try:
    tx_hash = darkPool.functions.trading_phase(0,0,0).transact({'from':w3.eth.accounts[1]})
except:
    test_num += 1
    tx_hash = darkPool.functions.trading_phase(0,0,0).transact()
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
else:
...
for i in range(2, clients+4):
    ...
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Register client gas used:", tx_receipt.gasUsed)
    ...
    # wait for the transaction to be mined
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Remove client gas used:", tx_receipt.gasUsed)
    ...
    # wait for the transaction to be mined
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Register client again gas used:", tx_receipt.gasUsed)
    assert darkPool.functions.us_pk(addr).call() == pk, "Test {} failed: Public-key not assigned correctly.".format(test_num)
    test_num += 1
    ...
    addr2keys[addr] = generate_key()
    pk = addr2keys[addr].public_key.format(True)
    # change public key of client
    tx_hash = darkPool.functions.register_client(addr, pk).transact()
    # wait for the transaction to be mined
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Chnage public-key gas used:", tx_receipt.gasUsed)
    assert darkPool.functions.us_pk(addr).call() == pk, "Test {} failed: New public-key not assigned correctly.".format(test_num)
    test_num += 1
...
try:
    tx_hash = darkPool.functions.trading_phase(1,123,0).transact()
except:
    test_num += 1
else:
...
# change phase
tx_hash = darkPool.functions.trading_phase(1,2,0).transact() # time, mode, deposit
# wait for the transaction to be mined
tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
if gas_verbose: print("Trading phase gas used:", tx_receipt.gasUsed)
assert darkPool.functions.phase().call() == 1, "Test {} failed: Trading Phase not initialised correctly.".format(test_num)
test_num += 1
# check auction mode
assert darkPool.functions.auctionMode().call() == 2, "Test {} failed: Auction mode not initialised correctly.".format(test_num)
test_num += 1
...
for i in range(2, clients+3):
    ...
    # wait for transaction receipt
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Change commitment gas used:", tx_receipt.gasUsed)
    # check if the commitment text on the contract matches the one send
    assert darkPool.functions.orders(addr).call()[0] == hash, "Test {} failed: Commitment received is incorrect.".format(test_num)
    test_num += 1
    ...
    # wait for transaction receipt
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Cancel commitment gas used:", tx_receipt.gasUsed)
    # check if the commitment text on the contract matches the one send
    assert darkPool.functions.orders(addr).call()[0] == b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', "Test {} failed: Commitment was not deleted.".format(test_num)
    test_num += 1
    ...
    # wait for transaction receipt
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Commit again gas used:", tx_receipt.gasUsed)
    # check if the commitment text on the contract matches the one send
    assert darkPool.functions.orders(addr).call()[0] == hash, "Test {} failed: Commitment received is incorrect.".format(test_num)
    test_num += 1

# check that the commitment is empty for a client who didn't send commitmnet
assert darkPool.functions.orders(w3.eth.accounts[1]).call()[0] == b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', "Test {} failed: Commitment should be empty.".format(test_num)
test_num += 1
...
# wait for the transaction to be mined
tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
if gas_verbose: print("Reveal phase gas used:", tx_receipt.gasUsed)
...
for i in range(2, clients+2):
    ...
    # wait for the transaction to be mined
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Reveal gas used:", tx_receipt.gasUsed)
    ...
    assert commitment == w3.solidityKeccak(['bytes'], [ciphertext]), "Test {} failed: Reported commitment is incorrect".format(test_num)
    test_num += 1
...
# wait for the transaction to be mined
tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
if gas_verbose: print("Calculation phase gas used:", tx_receipt.gasUsed)
...
for i in range(2, clients+2):
    ...
    assert commitment == w3.solidityKeccak(['bytes'], [ciphertext]), "Test {} failed: Received commitment doesn't match with received ciphertext.".format(test_num)
    test_num += 1
...
for i in range(2, clients+1):
    ...
    # wait for the transaction to be mined
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Reveal match gas used:", tx_receipt.gasUsed)
    ...
    # check if 1 or 2 secretRevealed events were emitted
    events = secret_filter.get_new_entries()
    assert len(events) == 1 or len(events) == 2, "Test {} failed: Contract should emit 1 or 2 secretRevealed events.".format(test_num)
    test_num += 1
    for e in events:
        ...
        assert commitment == w3.solidityKeccak(['bytes'], [ciphertexts[sender]]), "Test {} failed: Reported commitment is incorrect".format(test_num)
        test_num += 1
...
# wait for the transaction to be mined
tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
if gas_verbose: print("Results phase gas used:", tx_receipt.gasUsed)
...
# wait for the transaction to be mined
tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
if gas_verbose: print("Registration phase gas used:", tx_receipt.gasUsed)
...
for i in range(2, clients+4):
    ...
    # wait for the transaction to be mined
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    if gas_verbose: print("Remove order gas used:", tx_receipt.gasUsed)
    assert darkPool.functions.orders(addr).call()[0] == b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', "Test {} failed: Commitment not reset correctly.".format(test_num)
    test_num += 1
...
if gas_verbose:
    ...
    # wait for the transaction to be mined
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    print("Second Trading phase gas used:", tx_receipt.gasUsed)
